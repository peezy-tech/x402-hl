...
// src/intents/client/index.ts
async function signDeclaredExecutionIntent(paymentPayload, paymentRequired, config) {
  ...
  const advertised = paymentRequired.accepts.some(
    (requirements) => hashPaymentRequirements(requirements).toLowerCase() === selectedHash.toLowerCase()
  );
  if (!advertised) {
  ...
  if (hashExecutionIntentTemplate(intent).toLowerCase() !== declaration.intentTemplateHash.toLowerCase()) {
...
function createExecutionIntentClientExtension(config) {
...
async function resolveIntentInput(declaration, paymentRequired, selected, config) {
...
function assertExpectedDomain(intent, expected) {
  ...
  if (getAddress4(intent.gateway) !== getAddress4(parsed.gateway)) {
